...
export async function makeSlot({creator, id, name, type, width, height, impression, ctr}) {
    const operation = 'makePlaceSlotTx'
    const args = {
        creator,
        id,
        name,
...
export async function makeSlotStatusChange({ slotID, status }) {
    console.log('makeSlotStatusChange, status', status)
    let operation = 'makeChangeSlotStatusTx'
    status = status == 'active' ? 1 : 0
    let args = {
        slotID,
        status

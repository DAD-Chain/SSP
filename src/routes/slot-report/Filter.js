...
import { transformData, translateData } from 'services/slot-report'
    ...
    const handleExport = async () => {
        ...
        request({
            ...
            const data = [[
                ...
                record.clicks,
                `${record.ctr} %`,
                record.revenue_received,
                `https://explorer.dad.one/test/pages/address_en.html?${record.receive_address}`,
            ])))
            exportFile(data)
        })
    }
    ...
    return (
        <Row gutter={24}>
            <Col {...ColProps} xl={{ span: 4 }} md={{ span: 8 }}>
                {getFieldDecorator('slot_name', { initialValue: slot_name })(<Search placeholder="Search by slot name"
                    size="large"
